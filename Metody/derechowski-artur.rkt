#lang racket
;pomoc od Dominik Hawryluk w pierwszej części zadania
...
(define (solve-ax+by=1 a b)
  ...
  (if (= b 0) ;wtedy a=1 bo a i b są względnie pierwsze
      (cons 1 0) ;gdy b=0 to a*x=1 czyli x=1, y może być też inne ale wyjdą większe liczby
      (let* ([r (modulo a b)] ;znajac bx'+ry'=1      
             [q (/ (- a r) b)];mamy ay'+b(x'-qy')=1 po przekształceniach
             [last (solve-ax+by=1 b r)])
        (cons (cdr last)                  ;nowe x
              (- (car last)               ;nowe y
                            (* q (cdr last)))))))
...
(define (RSA-unconvert-list intlist key)
  ...
  (let ((n (key-modulus key)))
    (define (convert l sum)
      (if (null? l)
          '()
          (let ((x (modulo (+ sum (RSA-transform (car l);pomoc od Dominik Hawryluk
                                                 key)) n)))
            #|RSA-convert zamieniał listę  (a,b,c) na listę
                                           (F(a), F(b-F(a)), F(c-F(b-F(a)))),
             czyli po odwróceniu unconvert (a, b-F(a), c-F(b-F(a)))
             żeby to poprawić trzeba dodać jeszcze poprzednią wartość (np. dla c F(b-F(a))|#
            (cons x (convert (cdr l) (car l))))))
    (convert intlist 0)))
;TEST
;(RSA-decrypt (RSA-encrypt "alamakota" (car test-key-pair1)) (cdr test-key-pair1))
...
(define (make-signed-message text signature)
  (cons text signature))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  ;;; XXX: Write your code here
  (let* ([encrypted-text (RSA-encrypt
                          message
                          recipient-public-key)]
         [signature (RSA-encrypt
                     (number->string (compress encrypted-text))
                     sender-private-key)])
    (make-signed-message encrypted-text signature)))
;TEST
;(encrypt-and-sign "alamakota" (key-pair-public test-key-pair1) (key-pair-private test-key-pair2))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  ;;; XXX: Write your code here
  (let* ([text (RSA-decrypt
                (message cyphertext)
                recipient-private-key)]
         [givenhash (RSA-decrypt
                     (signature cyphertext)
                     sender-public-key)]
         [myhash (number->string (compress (message cyphertext)))])
    (if (string=? (string-trim givenhash) myhash)
#|string-trim jest potrzebny bo givenhash po zaszyfrowaniu i odszyfrowaniu RSA będzie
zawsze miał długość 4*n (długość bloku konwertującego stringa na listę intów|#
        text
        (error "wrong hash" givenhash myhash))))
;TEST
;(authenticate-and-decrypt (encrypt-and-sign "alamakota" (key-pair-public test-key-pair2) (key-pair-private test-key-pair1)) (key-pair-private test-key-pair2) (key-pair-public test-key-pair1))
