...
(define (expr? t)
  (or (const? t)
      ...
      (and (lambda? t)
           (expr? (lambda-expr t)))
      (exp-null? t);CWICZENIE 3
      (and (exp-null-pred? t)
           (expr? (exp-null-pred-exp t)))
      (and (exp-pair-pred? t)
           (expr? (exp-pair-pred-exp t)))
      (and (app? t)
           (expr? (app-proc t))
...
;CWICZENIE 3 (NULL, PAIR, LIST) robimy predykaty i konstruktory w naszym języku dla
;null, null?, pair, pair?
(define (exp-null? e)
  (eq? e 'null))
...
;CWICZENIE 4 (definiowanie w języku true, false, and, or)
(define exp-true 'true)

(define (exp-true? e)
  (eq? e exp-true))

(define exp-false 'false)
...
(define (eval-env e env)
  (writeln e)
  (cond [(exp-null? e) null]
    ;CWICZENIE 3 
        [(exp-null-pred? e) (null? (eval-env (exp-null-pred-exp e) env))]
        [(exp-pair-pred? e) (pair? (eval-env (exp-pair-pred-exp e) env))]
        [(exp-false? e) false]
        [(exp-true? e) true]
        [(exp-list? e) (eval-env (exp-list->cons (exp-list-args e)) env)]
        [(exp-if? e) (if (second e)
        ...
        [(lambda? e)
         ;(writeln (closure-cons (lambda-vars e) (lambda-expr e) env))
